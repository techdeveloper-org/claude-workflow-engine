...
| C9 | `prompt_gen_expert_caller.py` + `step_wrappers_0to4.py::step0_task_analysis_node` (**corrected during FR-3 implementation** — `00-prompt-generation/prompt_generator.py`/`TaskAnalyzer` originally listed here is dead code; nothing in the pipeline imports or calls it. The real live Step-0 LLM call is `prompt_gen_expert_caller.py`, invoked from `step_wrappers_0to4.py`'s "PRE-INJECTION" block, filling `templates/orchestration_system_prompt.txt`) | `.../architecture/prompt_gen_expert_caller.py`, `.../nodes/step_wrappers_0to4.py`, `.../templates/orchestration_system_prompt.txt` | **CHANGE** | FR-3 | Add a `routing` pre-injection (mirrors the existing CallGraph pre-injection pattern) computed before the LLM call; pass it through as a new template placeholder so `status=="resolved"` grounding reaches the LLM as authoritative context. Template itself is also stale (hardcodes "28 domains, 106 agents, 17 patterns" with no actual domain table embedded) — out of scope to fully modernize under FR-3, but the new placeholder must not perpetuate that gap for the fields it controls. |
| C10 | `todo_decomposer.py` | `.../architecture/todo_decomposer.py` | **KEEP (corrected — no code change needed)** | FR-3 | `todo_decomposer` only decomposes whatever `orchestration_prompt` text it's given; once C9's `orchestration_prompt` already contains the resolved agent/skills via the new routing block, decomposition picks it up with no separate code change. Re-verify this holds after C9 lands; only touch this file if the decomposition prompt template needs an explicit pointer to the routing block. |
