...
def tool_search(
    ...
    try:
        ...
        result += "\n".join(
            f"{path}: {count} matches" for path, count in sorted_results[:max_results]
        )
        return result
...
def find_key_files(root: str) -> Dict[str, List[str]]:
    ...
    try:
        ...
        for pattern in config_patterns:
            ...
            key_files["Config"].extend(
                [f.relative_to(root_path).as_posix() for f in files]
            )

        for pattern in main_patterns:
            files = list(root_path.rglob(pattern))
            key_files["Main/Entry"].extend(
                [f.relative_to(root_path).as_posix() for f in files]
            )

        for pattern in test_patterns:
            files = list(root_path.rglob(pattern))
            key_files["Tests"].extend(
                [f.relative_to(root_path).as_posix() for f in files]
            )

        for pattern in doc_patterns:
            files = list(root_path.rglob(pattern))
            key_files["Documentation"].extend(
                [f.relative_to(root_path).as_posix() for f in files]
            )
