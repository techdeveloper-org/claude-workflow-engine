...
class Level3JiraWorkflow:
    ...
    def _try_transition(
        ...
        try:
            ...
            logger.warning(
                "[JiraWorkflow] Could not fetch transitions for %s: %s",
                issue_key, exc
            )
            available = {}
        ...
        for candidate in transition_candidates:
            ...
            try:
                ...
                logger.info(
                    "[JiraWorkflow] Transitioned %s via '%s'",
                    issue_key, canonical
                )
                return {"transitioned": True, "transition_name": canonical, "error": ""}
            except Exception as exc:
                logger.warning(
                    "[JiraWorkflow] Transition '%s' failed for %s: %s",
                    canonical, issue_key, exc
                )

        names_tried = ", ".join(transition_candidates)
        logger.warning(
            "[JiraWorkflow] No matching transition found for %s "
            "(tried: %s; available: %s)",
            issue_key,
            names_tried,
        ...
        try:
            ...
            if github_issue_url and jira_key:
                ...
                try:
                    ...
                    logger.debug(
                        "[JiraWorkflow] Added GitHub cross-link comment to %s", jira_key
                    )
                except Exception as comment_exc:
                    logger.warning(
                        "[JiraWorkflow] Could not add cross-link comment to %s: %s",
                        jira_key, comment_exc
                    )
    ...
    def step11_link_pr_and_transition(
        ...
        """
        logger.info(
            "[JiraWorkflow] Step 11 - Link PR #%d to %s", pr_number, jira_issue_key
        )
        ...
        try:
            ...
            link_error = str(exc)
            logger.warning(
                "[JiraWorkflow] Could not link PR to %s: %s", jira_issue_key, exc
            )

        # 2. Transition issue to "In Review" (try candidates)
        candidates = [transition_name] + [
            c for c in IN_REVIEW_TRANSITIONS if c != transition_name
        ]
        ...
        try:
            ...
            logger.warning(
                "[JiraWorkflow] Could not add PR comment to %s: %s",
                jira_issue_key, comment_exc
            )
    ...
    def step12_close_jira_issue(
        ...
        try:
            ...
            logger.warning(
                "[JiraWorkflow] Could not add closing comment to %s: %s",
                jira_issue_key, exc
            )
