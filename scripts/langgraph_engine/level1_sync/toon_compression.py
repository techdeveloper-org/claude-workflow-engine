"""Level 1 Sync - TOON Compression node.

Canonical location: langgraph_engine/level1_sync/toon_compression.py
...
# Conditional imports
try:
    from .helpers import toons
except ImportError:
    toons = None  # type: ignore[assignment]

try:
    from .helpers import validate_toon
except ImportError:
    validate_toon = None  # type: ignore[assignment]
...
def node_toon_compression(state):
    ...
    try:
        ...
        schema_errors = []
        if _TOON_SCHEMA_AVAILABLE and validate_toon is not None:
        ...
        if session_path and str(session_path) != ".":
            try:
                ...
                if _TOONS_AVAILABLE and toons is not None:
                    try:
