...
class Level3FigmaWorkflow:
    ...
    def _get_tools(self):
        ...
        if self._figma_tools is None:
            try:
                src_mcp_path = str(
                    Path(__file__).resolve().parent.parent.parent / "src" / "mcp"
                )
    ...
    def step3_extract_components(self, file_key: str) -> Dict[str, Any]:
        ...
        try:
            ...
            pages = [
                p.get("name", "")
                for p in file_info.get("document", {}).get("children", [])
                if p.get("name")
            ]
    ...
    def step7_extract_design_tokens(
        self, file_key: str, node_ids: str = ""
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if raw_styles:
                named_colors = [
                    s.get("name", "") for s in raw_styles if s.get("styleType") == "FILL"
                ]
                named_text = [
                    s.get("name", "") for s in raw_styles if s.get("styleType") == "TEXT"
                ]
                if named_colors:
            ...
            logger.info(
                "[FigmaWorkflow] Extracted tokens: %d colors, %d typography, "
                "%d spacing, %d radii, %d shadows",
                len(colors), len(typography), len(spacing), len(radii), len(shadows),
            )
            return {
                "success": True,
    ...
    def step11_design_review(
        self, file_key: str, implementation_summary: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if colors:
                ...
                review_items.append(
                    "Colors match Figma spec: {} {}".format(
                        color_list, "(+more)" if len(colors) > 6 else ""
                    ).strip()
                )
            ...
            if typography:
                ...
                review_items.append(
                    "Typography matches Figma: {}{}".format(
                        ...
                        "and {} more style(s)".format(len(typography) - 1)
                        if len(typography) > 1 else "style applied",
                    )
                )
            ...
            if shadows:
                review_items.append(
                    "Box shadows applied: {} shadow(s) from Figma spec".format(len(shadows))
                )
            ...
            logger.info(
                "[FigmaWorkflow] Design review checklist: %d items", len(review_items)
            )
            return {
                "success": True,
...
def _build_prompt_snippet(tokens: Dict[str, Any]) -> str:
    ...
    if radii:
        lines.append("Border Radius: {}".format(
            ", ".join("{}px".format(r) for r in radii)
        ))
