...
Usage:
  ...
  # Before LLM call
  cached = rag.lookup(step="step0", query="bug fix in authentication module")
  if cached:
      return cached["decision"]  # Skip LLM call
...
def rag_lookup_before_llm(
    ...
    # type: (...) -> Optional[Dict]
    """Convenience function: lookup RAG before an LLM call.

    Uses state dict to extract session_id, project, context.
    Returns cached decision or None.
    """
    ...
    context = {
        "task_type": state.get("step0_task_type", state.get("task_type", "")),
        "complexity": state.get("step0_complexity", state.get("complexity_score", 0)),
        "framework": state.get("detected_framework", ""),
    }

    return rag.lookup(step=step, query=query, context=context)
