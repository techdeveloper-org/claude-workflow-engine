...
class Level3DocumentationManager:
    ...
    def detect_project_docs(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                "Doc detection: fresh=%s, found=%s",
                is_fresh, result["docs_found"]
            )
            return result
    ...
    def create_all_docs(self, state: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            result = generator.update_all_documentation(
        ...
        try:
            ...
            if claude_md_path.is_file():
                self._update_claude_md(
                    claude_md_path, session_id, task_type, complexity,
                    skill, agent, modified_files, date_str
                )
            ...
            self._update_changelog(
                changelog_path, task_type, modified_files,
                date_str, state
            )
            ...
            logger.info(
                "Existing project: updated %d docs: %s",
                len(updated_files), updated_files
            )
    ...
    def _update_claude_md(
        self, path: Path, session_id: str, task_type: str,
        complexity: int, skill: str, agent: str,
        modified_files: List[str], date_str: str
    ):
        ...
        try:
            ...
            if modified_files:
                insight_lines.append(
                    "- **Files Modified**: %d" % len(modified_files)
                )
    ...
    def _update_changelog(
        self, path: Path, task_type: str,
        modified_files: List[str], date_str: str, state: Dict
    ):
        ...
        try:
            ...
            category = "Added" if task_type.lower() in (
                "feature", "new feature"
            ) else "Changed" if task_type.lower() in (
                "enhancement", "refactor", "refactoring"
            ...
            if path.is_file():
                ...
                if insert_pos >= 0:
                    new_content = (
                        existing[:insert_pos] + entry + existing[insert_pos:]
                    )
                else:
    ...
    def _apply_doc_requirements(
        self, requirements: Dict, state: Dict
    ) -> List[str]:
        ...
        try:
            ...
            for file_spec in files_to_update:
                if isinstance(file_spec, str):
                    ...
                    if fpath.is_file():
                        logger.info(
                            "Standards doc requirement: %s (exists, skipping "
                            "auto-update - manual review needed)", file_spec
                        )
        except Exception as e:
