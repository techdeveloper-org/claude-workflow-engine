...
def suggest_test_scope(
    ...
    try:
        ...
        all_references = _collect_all_references(root)
        existing_tests_to_run = _find_tests_for_modified_files(
            root, modified_rel, all_references
        )
        ...
        for td in test_dirs:
            for tf in _collect_test_files(td):
                ...
                for stem in modified_stems:
                    ...
                    if (
                        tf_stem == "test_" + stem_lower
                        or tf_stem == stem_lower + "_test"
                        or stem_lower in tf_stem
                    ):
                        relevant.add(tf_str)
                        break
...
def generate_coverage_report(project_root, call_graph=None):
    ...
    try:
        ...
        fully_tested = [
            f for f, s in by_file.items()
            if s["methods"] > 0 and s["pct"] >= 100.0
        ]
        zero_coverage = [
            f for f, s in by_file.items()
            if s["methods"] > 0 and s["tested"] == 0
        ]
