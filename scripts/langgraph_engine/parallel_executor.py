...
class ParallelExplorer:
    ...
    def explore(
        ...
        logger.info(
            "[ParallelExplorer] Starting {} tasks with {} workers".format(
                len(tasks), workers
            )
        )
        ...
        with ThreadPoolExecutor(max_workers=workers, thread_name_prefix="explore") as pool:
            ...
            for fut in as_completed(future_to_idx, timeout=timeout_seconds * len(tasks)):
                ...
                try:
                    ...
                    logger.debug(
                        "[ParallelExplorer] '{}' done in {:.2f}s".format(label, elapsed)
                    )
                    ...
                    logger.warning(
                        "[ParallelExplorer] '{}' failed: {}".format(label, exc)
                    )
        ...
        logger.info(
            "[ParallelExplorer] {}/{} tasks succeeded".format(successful, len(tasks))
        )
            ...
            with ThreadPoolExecutor(max_workers=workers, thread_name_prefix="task") as pool:
                ...
                for fut in as_completed(future_to_task, timeout=timeout_seconds * len(batch)):
                    ...
                    try:
                        ...
                        logger.warning(
                            "[ParallelTaskExecutor] Task '{}' failed: {}".format(task_id, exc)
                        )

        total_elapsed = time.monotonic() - total_start
        successful = sum(1 for r in results_map.values() if r["success"])
        logger.info(
            "[ParallelTaskExecutor] {}/{} tasks succeeded in {:.2f}s".format(
                successful, len(tasks), total_elapsed
            )
        )

        # Return in original order
        return [results_map.get(t["id"], {
            "id": t["id"],
            "success": False,
            "result": None,
            "elapsed_s": 0.0,
...
class ConcurrentSkillDownloader:
    ...
    def load_skills(
        ...
        logger.info(
            "[ConcurrentSkillDownloader] Loading {} skills with {} workers".format(
                len(skill_specs), workers
            )
        )
        ...
        def _load_one(spec: Dict[str, Any]) -> None:
            ...
            try:
                ...
                logger.warning(
                    "[ConcurrentSkillDownloader] '{}' failed: {}".format(name, exc)
                )
                with lock:
                    results[name] = ""
        ...
        logger.info(
            "[ConcurrentSkillDownloader] {}/{} skills loaded in {:.2f}s".format(
                loaded, len(skill_specs), elapsed
            )
        )
        return results
