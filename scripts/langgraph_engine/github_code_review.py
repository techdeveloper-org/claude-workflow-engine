...
def check_java_spring_patterns(diff_text: str) -> List[str]:
    ...
    if "@Component" in diff_text and "@Autowired" not in diff_text:
        issues.append(
            "Warning: Found @Component but no @Autowired - verify dependency injection"
        )
...
def check_docker_best_practices(diff_text: str) -> List[str]:
    ...
    if "RUN apt-get" in diff_text:
        issues.append(
            "Warning: RUN apt-get without apt-get update - use multi-stage or combine"
        )
...
def run_code_review(
    ...
    try:
        ...
        if (
            "python-backend-engineer" in selected_skills
            or "python-backend-engineer" in selected_agents
        ):
            issues.extend(check_python_best_practices(diff_text))

        if (
            "spring-boot-microservices" in selected_skills
            or "spring-boot-microservices" in selected_agents
        ):
        ...
        if review_criteria is not None:
            ...
            try:
                ...
                if not eval_result.passed:
                    logger.warning(
                        "[CodeReview] ReviewCriteria FAILED - merge will be blocked"
                    )

            except Exception as criteria_err:
                logger.warning(
                    f"[CodeReview] ReviewCriteria evaluation error (non-fatal): {criteria_err}"
                )
