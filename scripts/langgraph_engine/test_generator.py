    ...
    try:
        ...
        if language == "java":
            ...
            if "src/main/java" in path_str:
                test_path = path_str.replace(
                    "src/main/java", "src/test/java", 1
                )
...
def _extract_methods_from_python_ast(file_path):
    ...
    methods = []
    try:
        source = Path(str(file_path)).read_text(encoding="utf-8",
                                                errors="replace")
...
def _resolve_methods_for_file(file_path, call_graph, language):
    ...
    if call_graph is not None:
        ...
        for node in nodes:
            ...
            if node_file and (node_file == fp
                              or fp.endswith(node_file)
                              or node_file.endswith(fp)):
                matched.append({
                    "name": node.get("name", ""),
                    "params": node.get("params", []),
                    "return_type": node.get("return_type", ""),
                    "parent_class": _extract_class_name(
                        node.get("parent_class", "")
                    ),
...
def _generate_python_tests(methods, class_name=None):
    ...
    for method in methods:
        ...
        if name in ("__init__", "__str__", "__repr__", "__del__",
                    "__enter__", "__exit__"):
            continue
        ...
        if class_name:
            # Class method tests
            lines.append(_PY_BASIC.format(
                method=name, class_name=class_name,
                default_args=default_args
            ))

            if cyclomatic > 1 and params:
                lines.append(_PY_NONE_INPUT.format(
                    method=name, class_name=class_name
                ))

            if cyclomatic > 3 and params:
                lines.append(_PY_EMPTY_INPUT.format(
                    method=name, class_name=class_name
                ))

            if cyclomatic > 5 and params:
                lines.append(_PY_BOUNDARY.format(
                    method=name, class_name=class_name
                ))

            if is_validate:
                lines.append(_PY_VALIDATE_TRUE.format(
                    method=name, class_name=class_name
                ))
                lines.append(_PY_VALIDATE_FALSE.format(
                    method=name, class_name=class_name
                ))

            if expected_type:
                lines.append(_PY_RETURN_TYPE.format(
                    method=name, class_name=class_name,
                    default_args=default_args, expected_type=expected_type
                ))
        else:
            # Module-level function tests
            lines.append(_PY_FUNC_BASIC.format(
                method=name, default_args=default_args
            ))
            if cyclomatic > 1 and params:
...
def _java_default_args(params):
    ...
    if not params:
        return ""
    return ", ".join("null  /* TODO: %s */" % p.split(":")[0].strip()
                     for p in params)
...
def _generate_java_tests(methods, class_name=None):
    ...
    for method in methods:
        ...
        lines.append(_JAVA_BASIC.format(
            Method=cap, method=name, default_args=default_args
        ))
...
def _ts_default_args(params):
    ...
    if not params:
        return ""
    return ", ".join("undefined /* TODO: %s */" % p.split(":")[0].strip()
                     for p in params)
...
def _generate_typescript_tests(methods, class_name=None):
    ...
    if class_name:
        lines.append(_TS_HEADER.format(
        ...
        lines.append(_TS_FUNC_HEADER.format(
            module=module_name, imports=imports_str
        ))
        for method in methods:
            name = method.get("name", "")
            params = method.get("params", [])
            ...
            if vis == "-":
                continue
            default_args = _ts_default_args(params)
            lines.append(_TS_FUNC_BASIC.format(
                method=name, default_args=default_args
            ))
        lines.append(_TS_CLOSE)
    ...
    try:
        ...
        if language == "python":
            for class_name, class_methods in by_class.items():
                parts.append(
                    _generate_python_tests(class_methods,
            ...
            for class_name, class_methods in by_class.items():
                parts.append(
                    _generate_java_tests(class_methods, class_name=class_name)
                )

        elif language in ("typescript", "javascript"):
            for class_name, class_methods in by_class.items():
                parts.append(
                    _generate_typescript_tests(class_methods,
                                               class_name=class_name)
                )
            if no_class:
                parts.append(
                    _generate_typescript_tests(no_class, class_name=None)
                )
...
def generate_tests_for_file(project_root, file_path, call_graph=None):
    ...
    try:
        ...
        if language == "python":
            ...
            header = _PY_HEADER.format(
                module=module_stem, import_line=import_line
            )
        ...
        methods_tested = sum(
            1 for m in methods
            if m.get("visibility") == "+"
            and m.get("name", "") not in (
                "__init__", "__str__", "__repr__", "__del__",
                "__enter__", "__exit__", "constructor", "toString",
                "hashCode", "equals"
            )
        )
...
def generate_tests_for_modified_files(project_root, modified_files,
                                       call_graph=None):
