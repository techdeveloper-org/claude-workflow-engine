            ...
            if isinstance(overrides, dict):
                for key, value in overrides.items():
                    if key in config:
                        ...
                        logger.debug(
                            "quality_gate: unknown config key ignored: %s", key
                        )
    ...
    try:
        ...
        if block_on_critical and gate["critical_count"] > 0:
            blocking_reasons.append(
                f"{gate['critical_count']} CRITICAL/BLOCKER finding(s)"
            )
        if block_on_major and gate["major_count"] > 0:
            blocking_reasons.append(
                f"{gate['major_count']} MAJOR finding(s)"
            )
        ...
        if blocking_reasons:
            ...
            if gate["major_count"] > 0 and not block_on_major:
                parts.append(
                    f"{gate['major_count']} major finding(s) (warning only)"
                )
    ...
    try:
        ...
        for key in (
            ...
            if candidate and isinstance(candidate, dict):
                coverage_before = float(
                    candidate.get("coverage_before", 0.0) or 0.0
                )
                coverage_after = float(
                    candidate.get("coverage_after_estimate", 0.0)
                    or candidate.get("coverage_after", 0.0)
                    or 0.0
                )
                break
        ...
        if coverage_before == 0.0 and coverage_after == 0.0:
            ...
            if suggest_fn is not None:
                ...
                coverage_after = float(
                    scope.get("coverage_after_estimate", 0.0) or 0.0
                )
            else:
        ...
        if effective_pct < threshold and threshold > 0:
            blocking_reasons.append(
                f"Coverage {effective_pct:.1f}% is below threshold {threshold:.1f}%"
            )
        if drop > drop_threshold:
            blocking_reasons.append(
                f"Coverage dropped {drop:.1f}% (max allowed: {drop_threshold:.1f}%)"
            )
        ...
        if blocking_reasons:
            ...
            gate["reason"] = (
                f"{effective_pct:.1f}% (threshold: {threshold:.1f}%)"
            )
            if drop > 0:
...
def _evaluate_breaking_changes_gate(
    ...
    try:
        ...
        breaking_changes: List[Dict[str, Any]] = (
            state.get("step11_breaking_changes") or []
        )
        risk_assessment: str = (
            state.get("step11_risk_assessment") or "safe"
        )
        ...
        if breaking_changes and not allow_breaking:
            methods_str = ", ".join(
                bc.get("method", "unknown") for bc in breaking_changes[:5]
            )
            suffix = (
                f" (+{len(breaking_changes) - 5} more)"
                if len(breaking_changes) > 5
                else ""
            )
            gate["passed"] = False
            gate["reason"] = (
                f"{len(breaking_changes)} breaking change(s) detected: "
                f"{methods_str}{suffix}"
            )
        elif breaking_changes and allow_breaking:
            gate["reason"] = (
                f"{len(breaking_changes)} breaking change(s) detected but "
                f"allow_breaking_changes=True"
            )
        elif risk_assessment in ("risky",) and not allow_breaking:
            gate["passed"] = False
            gate["reason"] = (
                f"Risk assessment is '{risk_assessment}' - manual review required"
            )
        else:
            gate["reason"] = (
        ...
        logger.warning(
            "quality_gate: breaking changes gate evaluation failed: %s", exc
        )
        gate["passed"] = True
        gate["reason"] = f"Gate evaluation error (fail-safe pass): {exc}"
...
def _evaluate_tests_exist_gate(
    ...
    try:
            ...
            if test_dir.is_dir():
                try:
                    for f in test_dir.rglob("*.py"):
                        ...
                        try:
                            test_file_names.add(
                                str(f.relative_to(root)).replace("\\", "/").lower()
                            )
                        except ValueError:
                            pass
                except OSError:
        ...
        gen_result = (
            state.get("step10_generated_tests")
            or state.get("step11_generated_tests")
            or {}
        )
        ...
        if without_tests and require_tests:
            ...
            suffix = (
                f" (+{len(without_tests) - 5} more)"
                if len(without_tests) > 5
                else ""
            )
            gate["passed"] = False
            gate["reason"] = (
                f"{len(without_tests)} modified file(s) missing tests: "
                + ", ".join(names)
                + suffix
            )
        elif without_tests and not require_tests:
            gate["reason"] = (
                f"{len(without_tests)} file(s) missing tests (warning; "
                f"require_tests_for_modified=False)"
            )
        else:
            gate["reason"] = (
                "All modified files have corresponding test files"
                if with_tests
                else "No Python source files to check"
            )

    except Exception as exc:
        logger.warning(
            "quality_gate: tests-exist gate evaluation failed: %s", exc
        )
        gate["passed"] = True
        gate["reason"] = f"Gate evaluation error (fail-safe pass): {exc}"
...
def evaluate_quality_gate(
    ...
    modified_files: List[str] = list(
        state.get("step10_modified_files") or []
    )
    ...
    sonar_gate = _evaluate_sonar_gate(
        project_root, state, effective_config, modified_files
    )
    coverage_gate = _evaluate_coverage_gate(
        project_root, state, effective_config, modified_files
    )
    breaking_gate = _evaluate_breaking_changes_gate(state, effective_config)
    tests_gate = _evaluate_tests_exist_gate(
        project_root, state, effective_config, modified_files
    )
    ...
    blocking_gates: List[str] = [
        name for name, result in gates.items() if not result.get("passed", True)
    ]
    ...
    elif breaking_gate.get("risk_assessment") == "risky" or (
        sonar_gate.get("critical_count", 0) > 0
    ):
        ...
        if not sonar.get("passed", True):
            ...
            if major_count > 0:
                suggestions.append({
                    "gate": "sonar",
                    "action": (
                        f"Review {major_count} MAJOR finding(s); "
                        f"fix or suppress with documented justification."
                    ),
                    "priority": "MEDIUM",
        ...
        if not coverage.get("passed", True):
            ...
            if current < threshold:
                ...
                suggestions.append({
                    "gate": "coverage",
                    "action": (
                        f"Coverage dropped {drop:.1f}%. "
                        f"Add tests for the newly added code paths."
                    ),
                    "priority": "MEDIUM",
