...
class SonarQubeScanner:
        ...
        try:
            ...
            if api_available and project_key:
                scan_result = run_sonar_scan(
                    project_root, modified_files, config=config
                )
                measures = get_project_measures(
                    project_key=project_key, config=config
                )
                quality_gate = get_quality_gate_status(
                    project_key=project_key, config=config
                )
                ...
                logger.debug(
                    "[SonarQubeScanner] SonarQube not available; using lightweight scanner"
                )
    ...
    def scan_basic(
        ...
        """
        logger.info(
            "[SonarQubeScanner] scan_basic() - project_root=%s", project_root
        )
        try:
            scan_result = run_basic_scan(project_root, modified_files)
            findings = scan_result.get("findings", [])
    ...
    def auto_fix(
        ...
        """
        logger.info(
            "[SonarQubeScanner] auto_fix() - %d findings", len(findings)
        )
