...
def _create_infra_objects(session_id: str) -> Dict[str, Any]:
    ...
    return {
        "checkpoint": LazyLoader.load(
            "scripts.langgraph_engine.checkpoint_manager",
            "CheckpointManager",
            session_id,
        ),
        "metrics": LazyLoader.load(
            "scripts.langgraph_engine.metrics_collector",
            "MetricsCollector",
            session_id,
        ),
        "error_logger": LazyLoader.load(
            "scripts.langgraph_engine.error_logger",
            "ErrorLogger",
            session_id=session_id,
        ),
        "backup": LazyLoader.load(
            "scripts.langgraph_engine.backup_manager",
            "BackupManager",
            session_id=session_id,
        ),
    }
