...
class SkillManager:
    ...
    def __init__(
        ...
        logger.info(
            f"[SkillManager] Initialized: skills_root={self.skills_root}, "
    ...
    def provision_skill(
        ...
        if not download_result["success"]:
            ...
            if disk_fallback:
                logger.warning(
                    f"[SkillManager] Download failed for '{skill_name}', "
                    f"falling back to disk cache"
                )
                return self._build_result(
                    skill_name=skill_name,
    ...
    def validate_skill_set(
        ...
        for skill_name, content in skill_contents.items():
            ...
            version_reqs[skill_name] = {
                d["name"]: d["version_req"] for d in meta["mandatory"]
            }
        ...
        valid = (
            len(circular_deps) == 0
            and version_result["valid"]
            and len(deprecation_warnings) == 0
        )
    ...
    def _download_with_retry(self, skill_name: str) -> Dict[str, Any]:
        ...
        for url in urls:
            for attempt in range(MAX_RETRIES + 1):
                ...
                if delay > 0:
                    logger.info(
                        f"[SkillManager] Retry {attempt}/{MAX_RETRIES} for '{skill_name}' "
                        f"(delay={delay}s)"
                    )
                    time.sleep(delay)
                ...
                if error_code == 404:
                    logger.debug(
                        f"[SkillManager] 404 for '{skill_name}' at {url}, "
                        f"trying next URL candidate"
                    )
                    break  # Try next URL candidate

                logger.warning(
                    f"[SkillManager] Download attempt {attempt + 1} failed for '{skill_name}': "
                    f"{last_error}"
                )
        ...
        for pattern in search_patterns:
            ...
            if matches:
                try:
                    ...
                    logger.debug(
                        f"[SkillManager] Loaded '{skill_name}' from disk: {matches[0]}"
                    )
                    return content
                except OSError as exc:
                    logger.warning(
                        f"[SkillManager] Failed to read '{matches[0]}': {exc}"
                    )
