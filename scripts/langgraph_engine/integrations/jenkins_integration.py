...
class JenkinsIntegration(AbstractIntegration):
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        self._state = (
            IntegrationState.READY if self.is_enabled
    ...
    def _build_auth_header(self) -> str:
        ...
        user = (
            self._config.get("jenkins_user", "")
            or os.environ.get("JENKINS_USER", "")
        )
        token = (
            self._config.get("jenkins_api_token", "")
            or os.environ.get("JENKINS_API_TOKEN", "")
        )
    ...
    def _api_get(self, path: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            with urllib.request.urlopen(
                req, timeout=_JENKINS_REQUEST_TIMEOUT, context=ctx
            ) as resp:
            ...
            logger.debug(
                "[JenkinsIntegration] GET %s failed: HTTP %d", path, exc.code
            )
            return None
        except urllib.error.URLError as exc:
            logger.debug(
                "[JenkinsIntegration] GET %s unreachable: %s", path, exc.reason
            )
            return None
        except Exception as exc:
            logger.debug(
                "[JenkinsIntegration] GET %s error: %s", path, exc
            )
            return None
    ...
    def _api_post(
        ...
        try:
            ...
            if exc.code == 201:
                return True
            logger.debug(
                "[JenkinsIntegration] POST %s failed: HTTP %d", path, exc.code
            )
            return False
        except urllib.error.URLError as exc:
            logger.debug(
                "[JenkinsIntegration] POST %s unreachable: %s", path, exc.reason
            )
            return False
        except Exception as exc:
            logger.debug(
                "[JenkinsIntegration] POST %s error: %s", path, exc
            )
            return False
    ...
    def _encode_job_path(self, job_name: str) -> str:
        ...
        encoded_parts = [
            urllib.parse.quote(p, safe="") for p in parts if p
        ]
    ...
    def _get_build_result(
        ...
        """
        job_path = self._encode_job_path(job_name)
        path = "{}/{}/api/json?tree=result,building".format(
    ...
    def _wait_for_build(
        ...
        while time.monotonic() < deadline:
            ...
            logger.debug(
                "[JenkinsIntegration] Build #%d still running; "
                "waiting %ds",
                build_number,
    ...
    def create(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        logger.debug(
            "[JenkinsIntegration] create() - no-op (Jenkins has no issue concept)"
        )
    ...
    def update(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(
            "[JenkinsIntegration] update() - Step 10: trigger build for "
            "job=%s branch=%s",
            job_name,
            branch_name,
        )
        ...
        try:
            ...
            if not triggered:
                ...
                return {
                    "success": False,
                    "reason": "Failed to trigger Jenkins build for {}".format(
                        job_name
                    ),
                }
            ...
            logger.error(
                "[JenkinsIntegration] update() failed: %s", exc
            )
            self._state = IntegrationState.ERROR
            return {"success": False, "reason": str(exc)}

    def on_review(
        self, pr_data: Dict[str, Any], context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        logger.info(
            "[JenkinsIntegration] on_review() - Step 11: validate build "
            "job=%s build=%d",
            job_name,
            build_number,
        )
        ...
        try:
            if build_number:
                ...
                job_path = self._encode_job_path(job_name)
                data = self._api_get(
                    "{}/lastBuild/api/json?tree=number,result,building".format(
                        job_path
                    )
                )
                if data is None:
            ...
            logger.error(
                "[JenkinsIntegration] on_review() failed: %s", exc
            )
            self._state = IntegrationState.ERROR
            return {"success": False, "reason": str(exc)}
    ...
    def close(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(
            "[JenkinsIntegration] close() - Step 12: post-merge build "
            "job=%s branch=%s pr=%s",
            job_name,
            default_branch,
            pr_number,
        ...
        try:
            ...
            if not triggered:
                ...
                return {
                    "success": False,
                    "reason": "Failed to trigger post-merge build for {}".format(
                        job_name
                    ),
                }
            ...
            logger.error(
                "[JenkinsIntegration] close() failed: %s", exc
            )
            self._state = IntegrationState.ERROR
            return {"success": False, "reason": str(exc)}
