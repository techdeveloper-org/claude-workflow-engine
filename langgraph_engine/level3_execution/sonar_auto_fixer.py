...
def generate_fix_instruction(
    ...
    # ------------------------------------------------------------------
        ...
        if source_line:
            ...
            template_fix = (
                indent_str + "# WARNING: eval/exec removed - use ast.literal_eval() for data "
    ...
    # ------------------------------------------------------------------
        ...
        if source_line and var_name:
            indent = len(source_line) - len(source_line.lstrip())
            indent_str = source_line[:indent]
            template_fix = indent_str + f'{var_name} = os.environ.get("{env_var}", "")'
        else:
            template_fix = None
        instruction = (
...
def apply_template_fix(
    ...
    if line_no > len(original_lines):
        result["error"] = f"line {line_no} is out of range " f"(file has {len(original_lines)} lines)"
        return result
    ...
    if p.suffix == ".py":
        try:
            ...
            try:
                ...
                logger.debug("sonar_auto_fixer: backup restore failed: %s", restore_exc)
...
def run_fix_loop(
    ...
    try:
        from . import sonarqube_scanner as _scanner
    except ImportError:
        try:
            from langgraph_engine import sonarqube_scanner as _scanner  # type: ignore[no-redef]
        except ImportError:
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        _seen_llm = {(f.get("file"), f.get("line"), f.get("rule")) for f in llm_needed}
    ...
    auto_fixable_in_plan = sum(1 for r in all_fixes_applied if r.get("fix_type") == "auto")
    auto_fixes_applied = sum(1 for r in all_fixes_applied if r.get("applied") is True)
...
def get_fix_summary(fix_result: Dict[str, Any]) -> str:
    ...
    if auto_failed > 0:
        ...
        lines.append("")
        failed_records = [f for f in fixes_applied if f.get("fix_type") == "auto" and not f.get("applied")]
        for rec in failed_records:
            err = rec.get("error") or "unknown error"
            lines.append(f"- `{rec.get('file', 'unknown')}` " f"line {rec.get('line', '?')}: {err}")
        lines.append("")
