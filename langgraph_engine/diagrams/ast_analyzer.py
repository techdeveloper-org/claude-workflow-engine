                ...
                if isinstance(item, ast.FunctionDef):
                    ...
                    for target in item.targets:
                        if isinstance(target, ast.Name):
                            attributes.append(make_attr_info(target.id, "", "+"))
                    ...
                    for stmt in ast.walk(item):
                        if isinstance(stmt, ast.Assign) and len(stmt.targets) == 1:
                            ...
                            if (
                                ...
                                # Avoid duplicates
                                existing = [a["name"] for a in attributes]
                                if attr_name not in existing:
                                    attributes.append(make_attr_info(attr_name, "", vis))

            classes.append(make_class_info(node.name, file_path, bases, methods, attributes))
    ...
    def extract_imports(self, file_path):
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                ...
                for alias in node.names:
                    result["from_imports"].append({"module": module, "name": alias.name})
