...
CLI usage:
    python langgraph_engine/metrics/dashboard.py --report
...
class MetricsDashboard:
    ...
    def __init__(self, benchmark_dir=None, output_dir=None):
        ...
        Args:
            benchmark_dir: Path to benchmark JSON files directory.
            output_dir:    Path to write metrics-summary.json (defaults to benchmark_dir).
        """
        if benchmark_dir:
    ...
    def load_benchmarks(self, limit=100):
        ...
        Args:
            limit: Maximum number of files to load (newest first).

        Returns:
            list of benchmark summary dicts, newest first.
        """
    ...
    def aggregate(self, benchmarks=None):
        ...
        Args:
            benchmarks: Pre-loaded list of benchmark dicts, or None to load from disk.

        Returns:
            dict with avg_total_time, per_step_avg, success_rates,
            error_distribution, etc.
        """
        ...
        overall_success_rate = round(total_successes / total_steps_run * 100, 1) if total_steps_run > 0 else 0
    ...
    def save_summary(self, metrics=None):
        ...
        Args:
            metrics: Pre-aggregated dict, or None to aggregate fresh.
    ...
    def format_report(self, metrics=None):
        ...
        lines = [
            ...
            "  Min/Max Time:       %.1f / %.1f s" % (metrics["min_total_time_s"], metrics["max_total_time_s"]),
            "  Overall Success:    %.1f%%" % metrics["overall_success_rate"],
            "  Avg LLM Calls:      %.1f" % metrics["avg_llm_calls"],
            "",
            "-" * 65,
            "  Step | Avg Time  | Success | Failures | Runs",
            "-" * 65,
        ]
...
def get_dashboard_data(days=7):
    ...
    Bridge function: reads from metrics.aggregator.get_full_report()
    ...
    """
    try:
        from langgraph_engine.metrics.aggregator import get_full_report
    ...
    if steps:
        ...
        for step_name in sorted(steps.keys()):
            ...
            if isinstance(step_data, dict):
                ...
                bar = "#" * bar_len
                lines.append("    %-20s  %s  %dms  %.0f%%" % (step_name, bar, avg_ms, rate * 100))
        lines.append("")
