...
def validate_level1_complete(state: FlowState) -> FlowState:
    ...
    if missing:
        if "warnings" not in state:
            state["warnings"] = []
        state["warnings"].append(f"Level 1 incomplete: missing {', '.join(missing)}")
...
def validate_java_project_consistency(state: FlowState) -> FlowState:
    ...
    if is_java and not java_standards:
        if "warnings" not in state:
            state["warnings"] = []
        state["warnings"].append("Java project detected but Java standards not loaded")
...
def validate_state_field(
    ...
    def _validate(state: FlowState) -> FlowState:
        ...
        if allowed_values and value not in allowed_values:
            if "errors" not in state:
                state["errors"] = []
            state["errors"].append(f"Field '{field_name}' value '{value}' not in allowed: {allowed_values}")
