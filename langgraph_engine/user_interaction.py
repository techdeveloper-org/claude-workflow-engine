    ...
    def confirm_with_user(
        ...
        try:
            return self.ask_user(
                ...
                risk_level=risk_level,
                context="Confirmation required before executing: %s" % action_description,
        ...
        try:
            for req in self._interactions:
                if req.get("id") == interaction_id:
                    ...
                    return True
            logger.warning("record_response: interaction_id %r not found", interaction_id)
            return False
        except Exception as exc:  # pragma: no cover
        ...
        try:
            ...
            for req in self._interactions:
                if req.get("status") == "pending":
                    ...
                    req["status"] = "answered"
                    count += 1
                    logger.debug("apply_defaults: %s -> %r", req.get("id"), fallback)
            if count:
                logger.info("InteractionManager.apply_defaults: applied %d default(s)", count)
            return count
        except Exception as exc:  # pragma: no cover
    ...
    try:
        ...
        for dep in unresolved[:_MAX_DEP_INTERACTIONS]:
            ...
            questions.append(
                make_interaction_request(
                    step=10,
                    question=("I found dependency '%s' but cannot find its source " "locally. Where is it?" % dep_name),
                    suggestion=(
                    ...
                    risk_level="low",
                    context=(
                        "Resolving internal dependencies improves call graph " "accuracy and prevents breaking changes."
                    ),
...
def generate_step11_questions(state: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        method_names = [b.get("method", "") if isinstance(b, dict) else str(b) for b in breaking[:5]]
        context_detail = ", ".join(m for m in method_names if m)

        questions.append(
            make_interaction_request(
                step=11,
                question=("%d breaking change(s) detected. Should I fix them before " "merging?" % len(breaking)),
                suggestion=("I recommend fixing breaking changes before merge to prevent " "regressions."),
                options=[
                ...
                risk_level="high",
                context=(
                    "Breaking changes: %s" % context_detail
                    if context_detail
    ...
    try:
        ...
        if complexity >= 7:
            questions.append(
                make_interaction_request(
                    step=13,
                    question=("This was a major change (complexity %d/10). " "Should I update all docs?" % complexity),
                    suggestion=(
                        "I recommend updating SRS, README, CHANGELOG, CLAUDE.md, " "and regenerating UML diagrams."
                    ),
                    options=[
                    ...
                    risk_level="low",
                    context=(
                        "Major changes often affect architecture docs. " "Keeping them in sync prevents confusion."
                    ),
                    fallback="Update all",
                )
            )
