...
Usage:
    ...
    python -m langgraph_engine.run_pipeline run \
        --message "Implement feature X" \
        --project .

    # Resume from the last checkpoint of a session
    python -m langgraph_engine.run_pipeline resume \
    ...
    python -m langgraph_engine.run_pipeline list \
        --session flow-abc12345

    # Short form: --resume flag (matches acceptance criteria)
    python -m langgraph_engine.run_pipeline --resume flow-abc12345:step-05
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    try:
        from langgraph_engine.orchestrator import invoke_flow
...
def cmd_resume(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if not session_id and not checkpoint_id:
            print("Error: --session is required for 'resume' command " "(optionally --checkpoint)", file=sys.stderr)
            return 1
...
def cmd_list(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        for cp in checkpoints:
            ...
            print(f"  Step {cp['step']:02d}  {status_str}  " f"{cp['timestamp']}  id={cp['checkpoint_id']}{err_suffix}")

        print()
        print("To resume from the last checkpoint:")
        print(f"  python -m langgraph_engine.run_pipeline resume --session {session_id}")
        if checkpoints:
            last = checkpoints[-1]
            print("To resume from a specific step:")
            print(
                f"  python -m langgraph_engine.run_pipeline resume "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Resume execution from the specified checkpoint ID " "(e.g. flow-abc12345:step-05)",
    )
    ...
    resume_parser.add_argument(
        ...
        "-c",
        required=False,
        help="Specific checkpoint ID (e.g. flow-abc12345:step-05). " "Defaults to last checkpoint.",
    )
