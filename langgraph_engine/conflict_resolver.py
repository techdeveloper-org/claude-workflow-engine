...
class ConflictResolver:
    ...
    def resolve_skill_conflicts(
        ...
        logger.info(f"[ConflictResolver] Skill resolution complete: " f"{len(working)} kept, {total_conflicts} removed")
    ...
    def resolve_standard_conflicts(
        ...
        logger.info(
            f"[ConflictResolver] Standard resolution complete: " f"{total_conflicts} setting conflicts resolved"
        )
    ...
    def resolve_branch_conflict(
        ...
        if local_exists or remote_exists:
            ...
            logger.warning(f"[ConflictResolver] Branch conflict resolved: " f"'{desired_branch}' -> '{resolved_name}'")
        else:
    ...
    def _resolve_explicit_conflicts(self, skills: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[str]]:
    ...
    def _resolve_pattern_conflicts(self, skills: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[str]]:
    ...
    def _resolve_exclusive_flags(self, skills: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[str]]:
    ...
    def _resolve_domain_priority(self, skills: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[str]]:
    ...
    def _branch_exists_remote(self, branch: str, repo_path: str) -> bool:
        ...
        code, stdout, _ = self._run_git(["ls-remote", "--heads", "origin", branch], repo_path)
        return code == 0 and branch in stdout
