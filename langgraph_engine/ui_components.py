...
from .progress_display import STEP_LABELS, ProgressDisplay, format_step_status
...
class PipelineUI:
    ...
    def step_started(self, step: int, label: Optional[str] = None) -> None:
        ...
        """
        if self._progress:
            self._progress.update_step(step=step, status="RUNNING", label=label)
    ...
    def step_done(
        ...
        """
        if self._progress:
            self._progress.update_step(step=step, status="DONE", duration_ms=duration_ms, detail=detail)
        label = STEP_LABELS.get(step, f"Step {step}")
        dur_str = f" ({duration_ms:.0f}ms)" if duration_ms else ""
        self._write(format_step_status(step, self.total_steps, label + dur_str, "DONE"))
    ...
    def step_skipped(self, step: int, reason: str = "") -> None:
        ...
        """
        if self._progress:
            self._progress.update_step(step=step, status="SKIPPED", detail=reason if reason else None)
        label = STEP_LABELS.get(step, f"Step {step}")
        reason_str = f" - {reason}" if reason else ""
        self._write(format_step_status(step, self.total_steps, label + reason_str, "SKIPPED"))
    ...
    def step_failed(
        ...
        """
        if self._progress:
            self._progress.update_step(step=step, status="FAILED", duration_ms=duration_ms, detail=error)
        label = STEP_LABELS.get(step, f"Step {step}")
        error_str = f" - {error}" if error else ""
        self._write(format_step_status(step, self.total_steps, label + error_str, "FAILED"))
    ...
    def show_error(
        ...
        if isinstance(error, Exception):
            formatted = self._error_formatter.format_from_exception(error, step=step, session_id=self.session_id)
        else:
            etype = error_type or "GENERIC"
            formatted = self._error_formatter.format(etype, detail=str(error), context=ctx)
...
def render_session_summary(
    ...
    errors = errors or []
    elapsed_str = f"{elapsed_seconds:.0f}s" if elapsed_seconds < 60 else f"{elapsed_seconds / 60:.1f}m"
    lines = [
        "",
        "Session Summary",
        "-" * 40,
