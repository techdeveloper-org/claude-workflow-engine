...
from langgraph_engine.standards.library_adapter import (
...
_STANDARD_SELECTOR_RULES_DIR = _STANDARD_SELECTOR_CLAUDE_HOME / "rules"
...
# project_type -> language-standards filename under ~/.claude/rules/.
...
def load_framework_standards(project_type: str, framework: str) -> List[Dict[str, Any]]:
    """Load framework standards from the global Claude rules directory.

    Looks in ~/.claude/rules/ (the single source of the Claude rules; the repo
    no longer keeps a docs/standards/ copy) for a framework-specific file, e.g.
    {project_type}-{framework}-standards.md or {framework}-standards.md.
    Present today: flask-standards.md, django-standards.md, and
    spring-boot-standards.md -- these three frameworks get this doc
    ...
    arch_dir = _STANDARD_SELECTOR_RULES_DIR
...
def load_language_standards(project_type: str) -> List[Dict[str, Any]]:
    ...
    matching file from ~/.claude/rules/ if one exists for that language.
    ...
    candidate = _STANDARD_SELECTOR_RULES_DIR / filename
    if candidate.exists():
        try:
            content = candidate.read_text(encoding="utf-8", errors="replace")
            lang.append(
...
def select_standards(project_path: str, session_id: str = "default") -> Dict[str, Any]:
    ...
    Detection order:
      ...
      7. load_language_standards()            (priority=1)
      8. LibrarySkillLanguageAdapter.load()   (priority=0.5 - lowest precedence)
      9. detect_conflicts() + resolve_conflicts()
    ...
    traceability["sources_checked"].append(
        {
            ...
            "locations": [
                display_path("rules", "{}-{}-standards.md".format(project_type, framework)),
                display_path("rules", "{}-standards.md".format(framework)),
            ],
        }
    )
    ...
    library_language_loaded = LibrarySkillLanguageAdapter().load(project_type)
    all_standards.extend(library_language_loaded)
    traceability["sources_checked"].append(
        {
            "source": "library_language_standards",
            "priority": PRIORITY_LIBRARY_LANGUAGE_SKILL,
            "loaded": len(library_language_loaded),
            "locations": [
                "claude-global-library sibling: skills/{skill-name}/SKILL.md (via ResourceResolver, language tier)"
            ],
        }
    )
    ...
    logger.log_decision(
        ...
        reasoning=(
            "custom({}) > team({}) > framework({}) > library_skill({}) > language({}) > "
            "library_language_skill({})".format(
                len(custom_loaded),
                len(team_loaded),
                len(framework_loaded),
                len(library_skill_loaded),
                len(language_loaded),
                len(library_language_loaded),
            )
        ),
    ...
    if conflicts:
        logger.log_error(
            ...
            recovery_action=(
                "Higher numeric priority wins: custom(4) > team(3) > framework(2) > "
                "library_skill(1.5) > language(1) > library_language_skill(0.5)"
            ),
