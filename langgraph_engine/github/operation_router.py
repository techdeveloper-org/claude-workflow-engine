...
from langgraph_engine.github.integration import GitHubIntegration

try:
    from langgraph_engine.github.mcp import PYGITHUB_AVAILABLE, GitHubMCP
except ImportError:
...
class GitHubOperationRouter:
    ...
    def __init__(self, use_mcp=True, fallback_to_gh=True, token=None, repo_path="."):
        """
        ...
        if use_mcp and PYGITHUB_AVAILABLE:
            try:
                ...
                logger.warning("[Router] MCP initialization failed: {}".format(e))
                self.mcp = None
        ...
        try:
            ...
            logger.error("[Router] gh CLI initialization failed: {}".format(e))
            self.gh_cli = None
    ...
    def create_issue(self, title, body="", labels=None, assignee=None):
        ...
        if self.use_mcp and self.mcp:
            try:
                ...
                if result.get("success"):
                    logger.info("[Router] create_issue: MCP succeeded (#{})".format(result["issue_number"]))
                    return result
                else:
                    logger.warning("[Router] create_issue: MCP failed, {}".format(result.get("error")))
            except Exception as e:
                logger.warning("[Router] create_issue: MCP error, {}".format(e))
        ...
        if self.fallback_to_gh and self.gh_cli:
            try:
                ...
                if result.get("success"):
                    logger.info("[Router] create_issue: gh CLI succeeded (#{})".format(result["issue_number"]))
                else:
                    logger.error("[Router] create_issue: gh CLI failed, {}".format(result.get("error")))
                return result
            except Exception as e:
                logger.error("[Router] create_issue: gh CLI error, {}".format(e))
        ...
        if self.use_mcp and self.mcp:
            try:
                ...
                if result.get("success"):
                    ...
                    logger.warning("[Router] add_issue_comment: MCP failed, {}".format(result.get("error")))
            except Exception as e:
                logger.warning("[Router] add_issue_comment: MCP error, {}".format(e))
        ...
        if self.fallback_to_gh and self.gh_cli:
            try:
                ...
                if result.get("success"):
                    ...
                    logger.error("[Router] add_issue_comment: gh CLI failed, {}".format(result.get("error")))
                return result
            except Exception as e:
                logger.error("[Router] add_issue_comment: gh CLI error, {}".format(e))
                return {"success": False, "error": str(e)}
        ...
        if self.use_mcp and self.mcp:
            try:
                ...
                if result.get("success"):
                    ...
                    logger.warning("[Router] close_issue: MCP failed, {}".format(result.get("error")))
            except Exception as e:
                logger.warning("[Router] close_issue: MCP error, {}".format(e))
        ...
        if self.fallback_to_gh and self.gh_cli:
            try:
                ...
                if result.get("success"):
                    ...
                    logger.error("[Router] close_issue: gh CLI failed, {}".format(result.get("error")))
                return result
            except Exception as e:
                logger.error("[Router] close_issue: gh CLI error, {}".format(e))
                return {"success": False, "error": str(e)}
    ...
    def create_pull_request(self, title, body="", head_branch=None, base_branch="main", labels=None):
        ...
        if self.use_mcp and self.mcp:
            try:
                ...
                if result.get("success"):
                    logger.info("[Router] create_pull_request: MCP succeeded (#{})".format(result["pr_number"]))
                    return result
                else:
                    logger.warning("[Router] create_pull_request: MCP failed, {}".format(result.get("error")))
            except Exception as e:
                logger.warning("[Router] create_pull_request: MCP error, {}".format(e))
        ...
        if self.fallback_to_gh and self.gh_cli:
            try:
                ...
                if result.get("success"):
                    logger.info("[Router] create_pull_request: gh CLI succeeded (#{})".format(result["pr_number"]))
                else:
                    logger.error("[Router] create_pull_request: gh CLI failed, {}".format(result.get("error")))
                return result
            except Exception as e:
                logger.error("[Router] create_pull_request: gh CLI error, {}".format(e))
                return {"success": False, "error": str(e)}
    ...
    def merge_pull_request(self, pr_number, commit_message=None, delete_branch=True):
        ...
        """
        logger.warning("[Router] merge_pull_request: CRITICAL destructive operation on PR #{}".format(pr_number))
        ...
        if self.use_mcp and self.mcp:
            try:
                ...
                if result.get("success"):
                    logger.warning("[Router] merge_pull_request: MCP succeeded (PR #{} merged)".format(pr_number))
                    return result
                else:
                    logger.warning("[Router] merge_pull_request: MCP failed, {}".format(result.get("error")))
            except Exception as e:
                logger.warning("[Router] merge_pull_request: MCP error, {}".format(e))
        ...
        if self.gh_cli:
            try:
                ...
                if result.get("success"):
                    logger.warning("[Router] merge_pull_request: gh CLI succeeded (PR #{} merged)".format(pr_number))
                else:
                    logger.error("[Router] merge_pull_request: gh CLI failed, {}".format(result.get("error")))
                return result
            except Exception as e:
                logger.error("[Router] merge_pull_request: gh CLI error (CRITICAL), {}".format(e))
                return {"success": False, "error": str(e)}

        # No fallback available - this is critical
        error_msg = "No GitHub backend available for critical merge operation"
        logger.error("[Router] merge_pull_request: {}".format(error_msg))
        ...
        if self.use_mcp and self.mcp:
            try:
                ...
                if result.get("success"):
                    ...
                    logger.warning("[Router] add_pr_comment: MCP failed, {}".format(result.get("error")))
            except Exception as e:
                logger.warning("[Router] add_pr_comment: MCP error, {}".format(e))
        ...
        if self.fallback_to_gh and self.gh_cli:
            try:
                ...
                if result.get("success"):
                    ...
                    logger.error("[Router] add_pr_comment: gh CLI failed, {}".format(result.get("error")))
                return result
            except Exception as e:
                logger.error("[Router] add_pr_comment: gh CLI error, {}".format(e))
                return {"success": False, "error": str(e)}
