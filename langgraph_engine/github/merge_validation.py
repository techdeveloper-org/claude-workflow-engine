...
def detect_git_conflict_markers(git_ops, branch_name):
    ...
    try:
        git_ops._run_git(
            ["merge", "--no-commit", "--no-ff", "origin/{}".format(branch_name)],
            check=False,
        )
        ...
        logger.warning("Error detecting git conflicts: {}".format(e))
        return []
...
def test_merge_locally(git_ops, branch_name):
...
def detect_project_type_for_validation(repo_path):
...
def _validate_python(git_ops):
    ...
    try:
        ...
        logger.warning("Python validation error: {}".format(e))
        return {"success": True}


def _validate_java(git_ops):
    ...
    try:
        ...
        logger.warning("Java validation error: {}".format(e))
        return {"success": True}


def _validate_nodejs(git_ops):
    ...
    try:
        ...
        logger.warning("Node.js validation error: {}".format(e))
        return {"success": True}


def _validate_go(git_ops):
    ...
    try:
        ...
        logger.warning("Go validation error: {}".format(e))
        return {"success": True}


def _validate_rust(git_ops):
    ...
    try:
        ...
        logger.warning("Rust validation error: {}".format(e))
        return {"success": True}


def validate_project_after_merge(git_ops, project_type):
    ...
    try:
        if project_type == "python":
            ...
            logger.info("Unknown project type '{}', skipping validation".format(project_type))
        ...
        return {
            "success": False,
            "reason": "Validation failed: {}".format(str(e)),
            "details": {},
        }
...
def check_merge_conflicts_bulletproof(github_router, git_ops, pr_number, branch_name):
    """Bulletproof merge conflict detection - 4 layers.
    ...
    try:
        ...
        logger.warning("[Layer 1] Could not check pr.mergeable: {}".format(e))
    ...
    if conflicts:
        logger.error("[Layer 2] FAILED: Found {} files with conflicts".format(len(conflicts)))
        return {
            "safe_to_merge": False,
            "layer": 2,
            "reason": "Git status: {} files have conflict markers".format(len(conflicts)),
    ...
    if not merge_test.get("success"):
        logger.error("[Layer 3] FAILED: {}".format(merge_test.get("reason")))
        return {
            "safe_to_merge": False,
            "layer": 3,
    ...
    logger.info("  Detected project type: {}".format(project_type))

    validation = validate_project_after_merge(git_ops, project_type)
    if not validation.get("success"):
        logger.error("[Layer 4] FAILED: {}".format(validation.get("reason")))
        return {
            "safe_to_merge": False,
            "layer": 4,
    ...
    return {
        ...
        "details": {
            ...
            "layer4": "Project validation passed ({})".format(project_type),
        },
    }
