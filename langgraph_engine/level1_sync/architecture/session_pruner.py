# -*- coding: utf-8 -*-
"""
session_pruner.py - Level 1 Sync System: Session Pruning Script
...
Usage:
    python session_pruner.py                  # Archive all eligible sessions
    python session_pruner.py --dry-run        # Preview without archiving
    python session_pruner.py --stats          # Show statistics only
    python session_pruner.py <session_dir>    # Archive specific sessions directory
...
def prune_sessions(
    ...
    for month_key, files in sorted(by_month.items()):
        ...
        try:
            ...
            _log_action("session_pruner", "archived", f"{month_key}: {len(files)} sessions")
...
def main() -> int:
    """CLI entry point for session_pruner.py."""
