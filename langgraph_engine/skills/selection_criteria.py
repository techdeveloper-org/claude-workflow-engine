...
def validate_skill(task: Dict[str, Any], skill: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    Example:
        ...
        validate_skill(task, skill)  # -> (True, "OK")
    """
...
def get_conflict_reason(skill1: Dict[str, Any], skill2: Dict[str, Any]) -> str:
    ...
    if skill1.get("exclusive"):
        return "'{}' is declared exclusive and cannot be combined".format(name1)
    if skill2.get("exclusive"):
        return "'{}' is declared exclusive and cannot be combined".format(name2)

    conflicts1 = [c.lower() for c in (skill1.get("conflicts_with") or [])]
    if name2.lower() in conflicts1:
        return "'{}' explicitly lists '{}' as a conflict".format(name1, name2)

    conflicts2 = [c.lower() for c in (skill2.get("conflicts_with") or [])]
    if name1.lower() in conflicts2:
        return "'{}' explicitly lists '{}' as a conflict".format(name2, name1)

    domain1 = skill1.get("exclusive_domain", "")
    if domain1:
        return "Both skills claim exclusive ownership of domain '{}'".format(domain1)
...
def detect_conflicts(selected_skills: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    Returns:
        List of conflict dicts, each with skill1 (str), skill2 (str), reason (str).
    ...
    Example:
        ...
        detect_conflicts(skills)  # -> [{"skill1": "flask", "skill2": "django", ...}]
    """
    ...
    for i, skill1 in enumerate(selected_skills):
        for skill2 in selected_skills[i + 1 :]:
            if not are_compatible(skill1, skill2):
                conflict = {
                    "skill1": skill1.get("name", "skill_{}".format(i)),
...
def score_skill(task: Dict[str, Any], skill: Dict[str, Any]) -> float:
    ...
    - up to +0.10  call graph hot-node bonus (capped to 1.0 total)
    """
    score = 0.0
    skill_caps: Set[str] = {c.lower() for c in (skill.get("capabilities") or [])}
    ...
    if required:
        ...
        score += 0.60 * (covered / len(required))
    else:
        score += 0.60
    ...
    if preferred:
        ...
        score += 0.20 * (covered_pref / len(preferred))
    else:
        score += 0.20
...
def build_selection(
    ...
    Returns:
        Dict with selected (List[Dict]), skipped_invalid (List),
        conflicts_found (List), scores (Dict[str, float])
    """
    ...
    scores = {skill.get("name", "skill_{}".format(i)): sc for i, (sc, skill) in enumerate(ranked)}
