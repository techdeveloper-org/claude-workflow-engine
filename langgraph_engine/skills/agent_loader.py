...
- list_all_skills() -> Dict[str, str]: Load all available skills with full definitions
- list_all_agents() -> Dict[str, str]: Load all available agents with full definitions
"""
...
try:
    from langgraph_engine.cache_system import cached_skill_load, get_pipeline_cache
    from langgraph_engine.parallel_executor import parallel_load_all_agents
            ...
            for sub_dir in skill_dir.iterdir():
                ...
                if sub_skill_file.exists():
                    try:
                        ...
                        logger.debug("Loaded skill (legacy path): %s (%d bytes)", sub_dir.name, len(content))
                    except Exception as e:
